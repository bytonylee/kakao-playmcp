...
docker run --detach --name minwon-run -p 18000:8000 minwon-run:local
...
docker run --detach --name naekkeo-recall -p 18001:8000 naekkeo-recall:local
...
Before deployment, run the repository secret check and dependency audits:
